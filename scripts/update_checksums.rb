...
def update_package_swift(version, checksums)
  ...
  # Dependency requirements are owned by update_package_dependencies.rb; keeping
  # a second copy of that logic here let the two drift apart.
  system('ruby', File.join(__dir__, 'update_package_dependencies.rb')) ||
    abort('Failed to sync dependency versions')
end
