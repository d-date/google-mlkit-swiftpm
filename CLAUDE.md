...
- `./scripts/verify_local_archive.sh` — the pre-release gate. Repoints `Package.swift` at `GoogleMLKit/`, **runs ML Kit on the arm64 Simulator** (text recognition, barcode, language ID), **launches the app** there, archives for device, and asserts every model bundle reached the app, nothing ITMS-scanned is embedded, nothing is linked dynamically and the model data survived. The runtime steps are the only ones that catch a missing model bundle or a bad simulator slice — a linking check cannot.
- `./scripts/verify_app_store_upload.sh [--upload]` — archives with distribution signing, exports an App Store IPA, validates it with Apple and optionally delivers it. ITMS-91065 is raised by App Store Connect's post-upload processing, not by a local export or by `altool --validate-app`, so confirming that class of problem needs the `--upload` run.
...
- **The `Resources/*-Info.plist` templates are shipped verbatim**, so a key missing there is missing in every consumer's app. `MLKitNaturalLanguage-Info.plist` had no `MinimumOSVersion`, and App Store Connect rejects an upload with "Invalid MinimumOSVersion … is \'\'" (90530) -- that blocked uploads for anyone using the language APIs. `verify_build.rb` now fails when a template is missing `MinimumOSVersion` or `CFBundleShortVersionString`, and all of them are pinned to the Makefile's `IPHONEOS_DEPLOYMENT_TARGET`.
